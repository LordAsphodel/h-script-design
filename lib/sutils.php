...
{
	$reg ? $p = mb_strpos($text, $ss) : $p = mb_stripos($text, $ss);
	if ($p === false) 
		return -1;
	else
		return $p;
...
{
	$reg ? $p = mb_strrpos($text, $ss) : $p = mb_strripos($text, $ss);
	if ($p === false) 
		return -1;
	else
		return $p;
...
function cutElemL(&$txt, $sep, $reg = true) 
{
	...
	{
		$s = textLeft($txt, $p);
		$txt = ltrim(mb_substr($txt, $p + textLen($sep)));
	}
	return trim($s);
}
...
function textLangFilter($text, $lng = '', $lt = '{!', $rt = '!}', $langs = array())
{
	...
	{
		$i2 = mb_strpos($text, $lt, $i);
		if ($i2 !== false)
		{
			if ($text{$i2 - 1} != '/')
			{
				$j = mb_strpos($text, $rt, $i2);
				if ($j !== false)
				{

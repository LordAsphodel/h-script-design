...
function scaleTo (f, t)
{
  ...
  {
    if (w >= 992) {
      var $h = Math.max(ff.outerHeight(), $(t).outerHeight());
      ff.css('height', $h+'px');
      $(t).css('height', $h+'px');
    }
    else {
...
radioClass: 'iradio_square-blue myaddclass-radio',
...
scaleTo('#cab-payment-info', '#cab-payment-info2');


if ($('#ref-share-box2').length > 0) {
  var ref_scale = setInterval(function () {
    var h = $('#ref-share-box2').outerHeight();
    if (h > 20) {
      scaleTo('#ref-share-box', '#ref-share-box2');
      clearInterval(ref_scale);
    }
  }, 10);
}
...
if($('#messages-scroll-bottom').length > 0) {
  $('.body.content.rows').scrollTop(10000);
  $('#dialog_text').focus();
}
...
var readAlerts = function () {
  ...
  $.ajax({
    method: "POST",
    url: "/ajax/read_alerts.php",
    data: {},
    success: function (msg) {

      console.log('send ajax request for reading my alerts');
      $('#alerts_counter').fadeOut(70);
